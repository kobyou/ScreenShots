...

{
	...
	//CString Msg;
	//Msg.Format("left:%d,top:%d,right:%d,bottom:%d,width:%d,height:%d", nX, nY, nX2, nY2, nWidth, nHeight);
	//MessageBox(Msg);

	// 确保选定区域不为空矩形
	if(((nWidth == 0) || (nHeight == 0)) ||
	        ((nWidth == 4) && (nHeight == 4))) {
		//MessageBox("Empty Rect!");
		return NULL;
	}
...
int CCatchScreenDlg::SaveHBITMAP(HBITMAP hB)
{
	BITMAP csBitmap;
	...
	//file name
	char fileDir[256] = { 0 };
	sprintf(fileDir, "C:\\Users\\%s\\Pictures\\ScrShots", cUser);
	if(_access(fileDir, 0) != 0) {
		_mkdir(fileDir);
	}

	sprintf(fileDir, "C:\\Users\\%s\\Pictures\\ScrShots\\%d%02d%02d%02", cUser, st.wYear, st.wMonth, st.wDay);
	if(_access(fileDir, 0) != 0) {
		_mkdir(fileDir);
	}

	char file_name[256] = { 0 };
	sprintf(file_name, "%s\\ScrShots_%02d%02d%02d.png", fileDir, st.wHour, st.wMinute, st.wSecond);
